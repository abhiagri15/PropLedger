...
def render_reports():
    ...
    # Sub-menu for Reports - added Properties Performance
    sub_selected = option_menu(
        None,
        ["P&L", "Transactions", "Properties Performance"],
        icons=["graph-up", "list-task", "house-door"],
        menu_icon="cast",
        default_index=0,
        key="reports_submenu",
    ...
    if sub_selected == "P&L":
        ...
        if 'selected_organization' in st.session_state and st.session_state.selected_organization:
            ...
            try:
                ...
                if org_properties:
                    st.info(f"Showing performance metrics for {org_name}'s properties")
                    ...
                    total_monthly_rent = sum(p.monthly_rent for p in org_properties)
                    total_income_all = sum(float(d['Total Income'].replace('$', '').replace(',', '')) for d in performance_data)
                    total_expenses_all = sum(float(d['Total Expenses'].replace('$', '').replace(',', '')) for d in performance_data)
                    total_net_income = total_income_all - total_expenses_all

                    with col1:
                        st.metric("Total Properties", len(org_properties))
                    with col2:
                        st.metric("Total Monthly Rent", f"${total_monthly_rent:,.2f}")
                    with col3:
                        st.metric("Total Net Income", f"${total_net_income:,.2f}")
                    with col4:
                        avg_roi = sum(float(d['ROI'].replace('%', '')) for d in performance_data) / len(performance_data) if performance_data else 0
                    ...
                    with col1:
                        # Income vs Expenses per property
                        property_names = [d['Property'] for d in performance_data]
                        income_values = [float(d['Total Income'].replace('$', '').replace(',', '')) for d in performance_data]
                        expense_values = [float(d['Total Expenses'].replace('$', '').replace(',', '')) for d in performance_data]

                        fig = go.Figure()
                        fig.add_trace(go.Bar(name='Income', x=property_names, y=income_values, marker_color='#2E8B57'))
                        fig.add_trace(go.Bar(name='Expenses', x=property_names, y=expense_values, marker_color='#DC143C'))
                        fig.update_layout(title="Income vs Expenses by Property", barmode='group')
                        st.plotly_chart(fig, use_container_width=True)

                    with col2:
                        # ROI comparison
                        roi_values = [float(d['ROI'].replace('%', '')) for d in performance_data]

                        fig = go.Figure()
                        fig.add_trace(go.Bar(x=property_names, y=roi_values, marker_color='#4169E1'))
                        fig.update_layout(title="ROI by Property (%)", yaxis_title="ROI %")
                        st.plotly_chart(fig, use_container_width=True)
                    ...
                    with col1:
                        if st.button("📄 Download PDF", key="download_prop_perf_pdf"):
                            st.info("PDF download feature coming soon!")

                    with col2:
                        if st.button("📊 Download Excel", key="download_prop_perf_excel"):
                            try:
                                import io
                                import base64
                                ...
                                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                                    # Properties sheet
                                    df_performance.to_excel(writer, sheet_name='Properties', index=False)

                                    # Summary sheet
                                    summary_df = pd.DataFrame([
                                        ['Total Properties', len(org_properties)],
                                        ['Total Monthly Rent', f"${total_monthly_rent:,.2f}"],
                                        ['Total Net Income', f"${total_net_income:,.2f}"],
                                ...
                                output.seek(0)
                                b64 = base64.b64encode(output.getvalue()).decode()
                                filename = f"{org_name.replace(' ', '_')}_Properties_Performance.xlsx"
                                st.markdown(f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{filename}">📊 Click to download Excel</a>', unsafe_allow_html=True)
                    ...
                    st.info(f"No properties found for {org_name}. Please add properties first.")

            except Exception as e:
                st.error(f"Error generating properties performance report: {str(e)}")
        else:
